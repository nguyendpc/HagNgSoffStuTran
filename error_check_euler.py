...
def error_from_sim(t,chirp):
    u0 = fromfile(file_prefix+str(float(chirp))+"_0.9_plots/frame000000.dat", dtype=float64)
    u = fromfile(file_prefix+str(float(chirp))+"_0.9_plots/frame" + sixdig(t)+".dat", dtype=float64)
    ue = fromfile(file_prefix_exact+str(float(chirp))+"_0.9_plots/frame" + sixdig(t)+".dat", dtype=float64)
    nrm = abs(ue*ue).sum()
