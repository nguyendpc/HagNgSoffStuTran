...
nguyentest = [0.5,0.75]
for b in nguyentest: #Anisotropy parameter
    f = (sqrt(1.0+b)+sqrt(1.0-b))/2.0
    g = (-1*sqrt(1.0+b)+sqrt(1.0-b))/2.0
    for fre in range(1,21):
        freq = float(fre)
       
        
        simname = "maxwell_orthotropic_exact_" + str(freq) + "_" + str(b)
        ...
        for i in range(int(tmax/dt)):
            u[0,int(npoints/2)-smallwidth:int(npoints/2)+smallwidth,int(npoints/2)-smallwidth:int(npoints/2)+smallwidth].real.tofile(os.path.join(simdir, "frame" + str(i).rjust(6,"0")+".dat"))
            time = i*dt
            print ("Time = " + str(time))
